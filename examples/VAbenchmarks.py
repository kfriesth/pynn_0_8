...
if spinnaker and profile:
    plot_start = 100
    plot_end = 200

    profiling_figure, profiling_axes = pylab.subplots(1, 3)

    print("E neural:")
    n_profiling_data = exc_cells.get_neural_profile_data()[0][1]
    sim.profiling.print_summary(n_profiling_data, tstop, dt)
    sim.profiling.plot_profile(
        sim.profiling.filter_time(n_profiling_data, plot_start, plot_end),
        profiling_axes[0])
    profiling_axes[0].set_title("Neural vertex")

    for i, (t, c) in enumerate(iteritems(exc_cells.get_synapse_profile_data())):
        print("E synapse type %s:" % str(t))
        sim.profiling.print_summary(c[0][1], tstop, dt)

        sim.profiling.plot_profile(
            sim.profiling.filter_time(c[0][1], plot_start, plot_end),
            profiling_axes[i + 1])
        profiling_axes[i + 1].set_title("Synapse vertex")

    profiling_axes[0].set_ylabel("Time [ms]")

    profiling_figure.tight_layout()
pylab.show()
